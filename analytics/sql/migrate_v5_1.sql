...
CREATE TABLE IF NOT EXISTS "positions"(
         ...
         gamma REAL,
         rho REAL,
         entry_iv REAL,  -- entry implied volatility
...
-- =========================================================
 CREATE TABLE IF NOT EXISTS "trades" (
         ...
         action TEXT CHECK(action IN ('BUY_OPEN','SELL_OPEN','BUY_CLOSE','SELL_CLOSE','ROLL')) NOT NULL,
...
-- =========================================================
-- 5. DATA NORMALIZATION / INFERENCE PATCHES
-- =========================================================
UPDATE positions
SET status = CASE
    WHEN contracts = 0 THEN 'CLOSED'
    WHEN contracts < (SELECT MAX(contracts) FROM positions AS p2 WHERE p2.symbol = positions.symbol)
         THEN 'PARTIAL'
    ELSE 'OPEN'
END;

UPDATE positions
SET iv = COALESCE(iv, entry_iv);

-- =========================================================
-- 6. SMOKE TEST SEED DATA (OPTIONAL)
-- =========================================================
